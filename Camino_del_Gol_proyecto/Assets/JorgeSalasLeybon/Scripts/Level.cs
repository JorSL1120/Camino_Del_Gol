...
public class Level : MonoBehaviour
{
    ...
    public void updateGuiSprite()
    {
        for (int j = 0; j < board.getSize(); j++)
        {
            for (int i = 0; i < board.getSize(); i++)
            {
                ...
                {
                    goBoard[i, j].GetComponent<SpriteRenderer>().sprite = spLevel;
                }
                else
                {
                    goBoard[i, j].GetComponent<SpriteRenderer>().sprite = spEnemy;
                }
            }
        }
    }
    ...
    {
        if (board.getFromBoard(pos.x, pos.y) > 0)
        {
            return true;
        }
